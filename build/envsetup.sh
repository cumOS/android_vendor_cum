function __print_cum_functions_help() {
cat <<EOF
Additional CumOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- cumgerrit:   A Git wrapper that fetches/pushes patch from/to CumOS Gerrit Review.
- cumrebase:   Rebase a Gerrit change and push it again.
- cumremote:   Add git remote for CumOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for CumOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Cum model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch cum_$target-$variant
        fi
    fi
    return $?
}
...
function cumremote()
{
    ...
    local CUM_USER=$(git config --get review.review.cumos.com.username)
    if [ -z "$CUM_USER" ]
    then
        git remote add cum ssh://review.cumos.com:29418/$PFX$PROJECT
    else
        git remote add cum ssh://$CUM_USER@review.cumos.com:29418/$PFX$PROJECT
    fi
    echo "Remote 'cum' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CumOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        cumremote
        git push cum HEAD:refs/heads/'$1'
    fi
    '
}
...
function cumrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "CumOS Gerrit Rebase Usage: "
        echo "      cumrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.cumos.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cum/build/tools/repopick.py $@
}
function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    common_target_out=common-${target_device}
    if [ ! -z $CUM_FIXUP_COMMON_OUT ]; then
