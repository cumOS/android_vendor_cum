...
func init() {
	android.RegisterModuleType("cum_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cumExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cumExpandVariables(ctx, dep_file)
	...
	exCmd := cumExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "cum.sbox.textproto"
	desc := "generate"
	name := "generator"
